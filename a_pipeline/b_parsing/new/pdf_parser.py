...
# ---------------------- LOAD PDF + META -------------------------------
doc_path      = Path("b_data/course_41554/document/files_pdf/41554_001_01_document.pdf")
...
for page_no, page in enumerate(md_pages):
    ...
    for para in filter(None, text.split("\n\n")):
        ...
        if imgs:
            generic_alt = re.compile(r'^(img|image|logo|figure|fig)[\d\-_]*$', re.I)

            def needs_ocr(alt: str) -> bool:
                alt = alt.strip()
                return (not alt) or generic_alt.match(alt) or len(alt) < 5
