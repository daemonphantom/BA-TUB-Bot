...
def crawl(driver, video_folder):
    ...
    print(f"Navigating to videos browse page: {browse_url}")
    ...
    print(f"Found {len(video_rows)} video rows on the page.")
    ...
    for row in video_rows:
        try:
            ...
            try:
                ...
                print(f"Found download link: {video_download_url}")
            except Exception as e:
                print("No download button found; attempting to extract video element src.")
                try:
                    video_elem = driver.find_element(By.CSS_SELECTOR, "video.vjs-tech[src*='.mp4']")
                    video_download_url = video_elem.get_attribute("src")
                    print(f"Found video element src: {video_download_url}")
                except Exception as e:
                    print(f"❌ Could not find a video download URL on page: {detail_url}")
                    driver.close()
                    driver.switch_to.window(main_tab)
                    continue
            ...
            # Download the video.
            print(f"⬇️ Downloading video from: {video_download_url}")
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Saved video as: {file_path}")
            else:
                print(f"❌ Error downloading video: HTTP {response.status_code} - {video_download_url}")
                driver.close()
                driver.switch_to.window(main_tab)
                continue
            ...
            print(f"❌ Exception processing video row: {e}")
